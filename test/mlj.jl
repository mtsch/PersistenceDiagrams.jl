...
diagrams = (
    dim_0=[PersistenceDiagram([(0, Inf), (0, 1)]),
           PersistenceDiagram([(0, Inf), (0, 1.5)])
           ],
    dim_1=[PersistenceDiagram([(0, 1), (1.2, 2)]),
           PersistenceDiagram([(1, 1.1), (1.3, 1.9)])
           ],
)
...
@testset "PersistenceImageVectorizer" begin
    for kwargs in (
        NamedTuple(),
        (sigma=0.1, slope_end=0.5, height=7),
        (sigma=0.1, width=6),
    )
